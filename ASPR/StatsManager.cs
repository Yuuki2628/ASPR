...
namespace ASPR
{
    class StatsManager
    {
        string[,] stats;
        string[,] personality;
        int enemiesCount = 206, personalityCount = 35;

        /// <summary>
        /// Returns the stats of an enemy or personality on a certain index
        /// </summary>
        /// <param name="position">index where the enemy/personality is located</param>
        /// <param name="statType">Enemies: 0:name, 1:hp, 2:diplomacy, 3:physical defence, 4:magical defence, 5: persuasion defence; Personalities: 0:name, 1: attack defence, 2: talk defence</param>
        /// <param name="type">e for enemy, p for personality</param>
        /// <returns></returns>
        public string this[int position, int statType, string type]
        {
            get {
                if (type == "e")
                    return stats[position, statType];
                else
                {
                    if (type == "p")
                        return personality[position, statType];
                    else return "no";
                }
            }
            private set { }
        ...
        public int personalitiesN
        {
            get => personalityCount;
        }
            ...
            stats = new string[,]
{
        ...
        private async Task Request()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                stats[enemiesCount - count + i, 0] = part[0];
                stats[enemiesCount - count + i, 1] = part[1];
                stats[enemiesCount - count + i, 2] = part[2];
                stats[enemiesCount - count + i, 3] = part[3];
                stats[enemiesCount - count + i, 4] = part[4];
                stats[enemiesCount - count + i, 5] = part[5];
            }
        }
